...
D-convexity/D-concavity is quite common in known family of copulas. As there is no explicit formula for the family of Gaussian copulas, it is difficult to prove its D-convexity or D-convexity. However, numerical approximations in the case $n=2$ seem to indicate that a Gaussian copula would be D-convex if its marginals are positively correlated, and D-concave if they are negatively correlated. Figure \ref{fig:gaussian_copula_simu} present those observations. The following proofs provide additional insight on the properties of famous families of copulas presented in table \ref{tab:familiy_of_copula}, in the case $n=2$.
\begin{proof}
...
\begin{figure}
    ...
    \begin{subfigure}{0.3\linewidth}
        \centering
        \includegraphics[width=\linewidth]{Images/Guassian_copula/gaussian_copula_3.png}
    \end{subfigure}\hfill
    \begin{subfigure}{0.3\linewidth}
        \centering
        \includegraphics[width=\linewidth]{Images/Guassian_copula/gaussian_copula_4.png}
    \end{subfigure}\hfill
    \begin{subfigure}{0.3\linewidth}
        \centering
        \includegraphics[width=\linewidth]{Images/Guassian_copula/gaussian_copula_5.png}
    \end{subfigure}
    \caption{Gaussian copula in the direction $u_1$ for a given $u_2$ and different correlations $r$ between $u_1$ and $u_2$.}
    \label{fig:gaussian_copula_simu}
\end{figure}

The rest of this section will present some remarks and usefull properties for D-convex and D-concave copulas.
...
\begin{proof}
    Let $(u_1,\dots,u_n)\in[0,1]^n$, $(v_1,\dots,v_n)\in[0,1]^n$ \st $\forall i\in\opi1,n\cli$, $u_i+v_i\leqslant 1$. Let $i\in\opi1,n\cli$. Applying the definition of convexity (\ref{eq:convex_copula}) with $v_i=0$ yields:
    \begin{eqnarray*}
    ...
    \end{eqnarray*}
    which proves equation \ref{eq:convex_sum}.

    Let $w_i=u_i-v_i\in [0,1]$, clearly $w_i+v_i\leqslant1$. Using equation \ref{eq:convex_sum}, it holds that:
    \begin{align*}
        ...
        &&&-C(u_1,\dots,v_i,\dots,u_n)
    \end{align*}
    which proves equation \ref{eq:convex_diff}.
\end{proof} 
...
\comroman{Je vois. Donc il faudrait définir $\M_{robust}$ uniquement comme dans l'équation \ref{eq:robust_ancestor}, et laisser tomber la contrainte de convexité si j'ai bien compris. Je t'en reparlerai lundi}
...
\end{eqnarray*}
and the marginal credal sets $\M(\pi_1)$, $\M(\pi_2)$ they induce. We note the focal sets as $a^1_1=\{x^1_2\},~a^1_2=\{x^1_2, x^1_3\},~a^1_3=\{x^1_1,x^1_2,x^1_3\}$ and $a^2_1=\{x^2_2\},~a^2_2=\{x^2_2, x^2_3\},~a^2_3=\{x^2_1, x^2_2, x^2_3\}$. By joining $\M(\pi_1)$ and $\M(\pi_2)$ using $C$, we can obtain the lower probability $\low$ of $\M_{robust}$ using equation \ref{eq:robust_set}, and a total of 6 belief functions $Bel^{\preceq_1,\preceq_2}_\times$ using equation \ref{eq:joint_mass} depending on the orders $\preceq_1, \preceq_2$ used to join the marginal masses. For instance if $\preceq_1$ is such that $a^1_3\preceq_1 a^1_1 \preceq_1 a^1_2$ and $\preceq_2$ such that $a^2_1 \preceq_2 a^2_2 \preceq_2 a^2_3$, then the bivariate mass $m_\times$ would be defined as follows:
\begin{eqnarray*}
...
If we consider the two events $E_1=\{x^1_2\}\times\{x^2_2, x^2_3\}$ and $E_2=\{x^1_2,x^1_3\}\times\{x^2_2\}$, it is possible to show that $\low(E_1)=\low(E_2)\approx0.131$ (by symmetry of the problem) which is obtained for $P_1(x^1_1)=0, P_1(x^1_2)=0.3, P_1(x^1_3)=0.7, P_2(x^2_1)=0.2, P_2(x^2_2)=0.3, P_2(x^2_3)=0.5$ for $E_1$, and similarly with $P_1$ and $P_2$ reversed for $E_2$. Those values were estimated by running simulations, but their exact value can be computed by solving an optimization problem as $C$ is differentiable (although it is a bit tedious to compute). Then for all orders $\preceq_1,~\preceq_2$ on the focal sets of $\pi_1$ and $\pi_2$ it holds:
...
\caption{Value of $Bel^{\preceq_1,\preceq_2}_\times$ for $E_1$ and $E_2$. Values in bald font represent the minimal value of the different belief functions, where $Bel^{\preceq_1,\preceq_2}_\times(E)=\low(E)$.}
