...
In the presence of multiple sources of uncertainty, the question of how to join those sources arises.\comloic{n'essaie pas de me le vendre Roman, sort ta machette non stérilisée je suis prêt c'est bon} Using probability distributions, the usual method for joining probability distributions is to use copulas (\Cref{sec:copulas})\comloic{alors là autant te dire ue j'aime autant parce que j'en viens et j'espère que c'était utile ^^'}. However, there are multiple ways of joining imprecise models using a copula\comloic{bein voyons}. Sections \ref{sec:robust_method} , \ref{sec:joint_mass} and \ref{sec:aggregation_method} present three different ways of joining credal sets using a copula. Those three methods are not equivalent, we thus explore their similarities and differences in \Cref{sec:inclusions_between_methods}. The methods used to join possibility distributions will serve to propagate the uncertainty in \Cref{chap:propagating}.
...
When working with multiple sources of uncertain information, one must take into account the dependency between the different sources in order to correctly determine the joint model. \Cref{sec:copulas} introduced copulas, a type of dependency model that we will be using for joining the uncertainty model on images intensities.\commanue{ça fait redite avec au dessus. A mon avis tu peux remonter et fusionner} 

When working with precise probabilities, using a copula $C$ to join univariate Cumulative Distribution Functions (CDF), called marginals, into a mutlivariate\commanu{multivariate} CDF is straightforward. When working with marginals modeled by Imprecise Probabilities, joining the models with a copula on the product space is not as trivial. We present here three different approaches to create a multivariate credal set using a copula and imprecise marginals\commanu{comme le dit manue, un peu redite avec l'intro, attention à pas répéter inutilement}. The first approach in \Cref{sec:robust_method} maintain the classical interpretation found\comloic{un found en trop je pense} of a copula found in Sklar's theorem, but is hard to compute. The second approach in \Cref{sec:joint_mass} takes some distances with the classical interpretation of a copula, but is easier to compute. Finally a third approach is presented in \Cref{sec:aggregation_method}, which completely abandons Sklar's theorem interpretation, but is very easy to compute. Inclusion relationships between those three approaches are explored in \Cref{sec:inclusions_between_methods} \commanu{quelle est ta contribution ? }.

We consider here $n\in\mathbb{N}^*$ uncertain variables $(X_i)_{1\leqslant i\leqslant n}$ taking values respectively in an\comloic{a ?} totally ordered finite space $\X_i$. Index $i$ will usually refer to the $i$-th random variable (or random set). We note $\M_i$ the credal set representing the uncertainty of $X_i$, and $C$ a $n$-copula. Focal sets of a belief function will be noted $a^i_k$, where $k$ refers to the $k$-th focal if they are numbered. We also note $\bigsqcup$ the union of disjoint elements. Finally, we must introduce the concept of cylindrical sets which will come in handy\commanu{c'est anglais come in handy?, j'enleverais juste "which will come in handy"} for specifying definition domains of various mappings. 
\begin{definition}[Cylindrical sets]
    Let $\X_1 \enum \X_n$ be $n$ sets and let $\X$\commanue{tu n'as pas un autre symbole car là je vais me mélanger. A moins que tu voulais mettre $\times$} be the Cartesian product of $\X_1 \enum \X_n$. We call a cylindrical (or cylinder) set $X$ of $\X$ a set which can be written as a Cartesian product of elements of $\X_1 \enum \X_n$, \ie:
    \begin{equation}
...
\Cref{fig:schema_m_robust} represents a schematic of how the lower probability $\low_{robust}$ of $\M_{robust}$ is obtained. First, two samples $F_1$ and $F_2$ (represented by ``$+$'' signs) are sampled from the marginals credal sets defined by $\Bel_1$ and $\Bel_2$. They are then joined using the copula $C$ to create a multivariate $CDF$ represented by a ``x'' sign. This process is then reproduced for every combination of samples in the marginal credal sets, and $\low_{robust}$ is the lower bound of those samples on every event. 
\begin{figure}[!ht]
    \centering
...
\begin{proof}
    ...
    There are multiple way of proving that $\sum_{A\subseteq\X}m_\times(A)=1$. A direct proof can be done in the case $n=2$, but the notations become quite heavy for any $n>2$. Instead let us use the interpretation of a copula as a multivariate CDF. This method will be also be\comloic{je te laisse voir quel "be" tu veux garder} used in future proofs.
    
    For all \(i\in\opi 0,\, n\cli\) let $F_i$ be a CDF over $[0,\, N_i]$, with, $F_i(j)=\sum_{k=0}^j m_i(a_k^i)$\commanu{bizarre les virgules dans cette phrase, je te laisse voir}. By Sklar's theorem, $F=C(F_1 \enum  F_n)$ is a multivariate CDF over $[0,\, N_1]\tdt[0,\, N_n]$. Thus, it holds that $F([0,\, N_1]\tdt[0,\, N_n])=1$ and\commanue{Alors là j'ai du mal avec les notation. Après c'est une preuve donc c'est pas grave}:
    \begin{eqnarray*}
...
\begin{example}\label{ex:joint_mass}
    ...
    $\bullet$ If "$a_1^1\preceq_1a_2^1$" is the arbitrary order (see \Cref{fig:joint_distrib_arb}), then
    \begin{eqnarray*}
...
We considered until now that an order has to be chosen arbitrarily. However, special cases of belief functions exhibit a natural order on their focal sets, for instance p-boxes and possibilities. Those special cases will be explored in \Cref{sec:inclusions_between_methods}.
...
\begin{definition}[Aggregated Credal Set]\label{def:aggregation_credal_set}
    Given a copula $C$ and $n$ marginal credal sets whose lower probabilities are $\low_1 \enum \low_n$, we define the credal set $\M_{agg}$ over the power set of $\X=\X_1\tdt X_n$ as\commanue{encore une fois tu ne mets qu'une borne alors que dans l'autre définition il y a deux}:
    \begin{eqnarray}
...
Constraints on this set only occur on cylindrical sets of product space $\X_1\tdt \X_n$ (from \cref{eq:cylindrical_sets})\commanue{je comprends pas ce que tu veux dire par constraints}, contrary to $\M_{mass}$ or $\M_{robust}$, we thus take the convex hull for extending its definition to probabilities on $\X$\commanue{en fait je ne comprends pas la phrase}. We note $\M_{agg}$ this credal set as it uses the copula as an aggregation operator only, without conserving the meaning associated to copulas by Sklar's theorem. In this regard\commanu{frenglish, respect plus anglais ?}, $\M_{agg}$ has less meaning than $\M_{robust}$ or $\M_{mass}$, but presents the advantage of being easier to compute on cylindrical sets. \Cref{fig:meaning_computation} sums up the performances\commanu{je mettrais singulier performance} of the different methods in terms of computation cost and meaningfulness\commanu{c'est bien cette synthèse visuel, par contre je ne sais pas comprendre pourquoi c'est plus ou moins expressif et plus ou moins difficile à calculer. peut etre le rappeler en 1 phrase, sauf bien comprendre les maths, ca fait un peu non justifié pourquoi c'est plus ou moins performant et corrélement moins expressif + cette fin de paragraphe ne serait elle pas d'une section/paragraphe à part sur cette question performance vs expressivité. C'est une question majeure pour l'applicabilité et elle est un peu mélangée, il me semble }.
...
\begin{proof}
    For $i\in\{1 \enum n\}$, let $\low_i$ be a lower probability avoiding sure lost\comloic{loss?}, \ie whose credal set $\M_i$ contains at least one probability distribution $P_i$. Let us define a multivariate probability $P$ on every $(A_1\tdt A_n)\subseteq\X$ as:
    \begin{eqnarray*}
...
\begin{proof}
    ...
    Then\commanue{je ne suis pas sure de comprendre pourquoi tu as besoin de passer par S}\comloic{petit bug d'affichage},
    \begin{eqnarray*}
...
\begin{proof}
    \Cref{eq:joint_mass_product} shows that the order on marginal focal sets does not matter in the case of the product copula. We thus show\commanue{have to show / need to show} that \cref{eq:joint_mass_product} holds.
    For simplicity and coherence with the notations of \cref{eq:hvolume}, we will note for all $i\in[0,n]$, $u^i=\sum_{k=0}^{k_i-1}m_i(a_k^i)$, $v^i_k=\sum_{k=0}^{k_i}m_i(a_k^i)$\commanue{il y a des trucs en indice et en exposant mais c'ets pas raccord avec la suite}. $\Pi_{i=1}^n\{u_i, v_i\}$ will refer to the Cartesian product $\{u_1, v_1\}\times\{u_2, v_2\}\tdt\{u_n, v_n\}$ and we will note $C_\Pi$ and $H$ as the product copula and its H-volume regardless of their number of marginals. Those notations established, it holds that:
    \begin{eqnarray*}
...
Necessity functions $\Nec$, also called minitive belief functions, are a special type of belief function verify\comloic{verifying ? that verify?} $\Nec(A\cap B) =\min(\Nec(A), \Nec(B))$ for all events $A, B$ in their domain of definition. In the multivariate case, when a belief\commanue{function?} is only defined on cylindrical sets, then the minitive property becomes:
\begin{eqnarray}
...
\begin{proof}
    If we denote by $\Bel_\times$ the belief function defined in \eqref{eq:joint_mass} where the ordering is the inclusion ordering $\preceq_i$ for $i\in[1,2]$. For convenience and with respects\commanu{respect?} to the notations of \cref{eq:hvolume}, we note: $u^i_k=\sum_{j=0}^{k}m_i(a_j^i)$ and consider that $a^i_0=\emptyset$.
...
\begin{example}\label{ex:necessity}
    ...
    \commanu{la je me suis dit exemple, je vais enfin comprendre un peu et ton exemple ne m'éclaire pas bcp, peut etre pas grave dans cette partie. J'aurais imaginé quelque chose de plus imagé et "concret" que des équations. Meme remarque que manue dans cette section, pas évident de faire le lien avec les pop d'exemple et le texte, à faire le liant pour aider un peu le lecteur déjà que c'est "dur" ;)}
\end{example}
...
\begin{example}\label{ex:pbox}\commanue{Une petite phrase avant pour éviter les pops d'exemple et de proposition.}
    ...
    \end{center}
    The p-boxes from \Cref{tab:example_pbox} lead to the same belief functions as in example \ref{ex:necessity}, thus leading to the same conclusions \ie $\M_{robust}\not\subseteq\M_{mass}$, $\M_{mass}\not\subseteq\M_{robust}$ and $\M_{agg}\not\subseteq\M_{robust}$\commanue{C'est pas les mêmes que dans la phrase juste avant l'example}. \commanu{idem que manue, voir le liant avec le texte, le popup arrive un peu comme ca sans savoir a quoi il se rapporte, aide le béotien svp ;)}
\end{example}

As stated previously, p-boxes are very closely related to CDF which can motivate one to apply \hyperref[theorem:sklar]{Sklar's Theorem} to the lower CDF and upper CDF respectively. Given $n$ p-boxes $[\underline{F}_1,~\overline{F}_1] \enum [\underline{F}_n,~\overline{F}_n]$ defined over $\X_1 \enum \X_n$ and a copula $C$, we can define the lower and upper bounds of a $n$ variate CDF as:
\begin{align*}
    \underline{F}_\times&=C(\underline{F}_1 \enum  \underline{F}_n)\\
    \overline{F}_\times&=C(\overline{F}_1 \enum  \overline{F}_n)
\end{align*}
Sklar's theorem states that $\underline{F}_\times$ and $\overline{F}_\times$ are both CDF, which means that $[\underline{F}_\times, \overline{F}_\times]$ is a multivariate p-box \cite{pelessoni_bivariate_2016, montes_sklars_2015} over cylindrical sets, defining a credal set $\M$. Clearly, the bounds of $\M_{robust}$ on cumulative events are the same as those of the credal set $\M$ induced by the multivariate p-box $[C(\underline{F}_1 \enum  \underline{F}_n),~C(\overline{F}_1 \enum  \overline{F}_n)]$\commanue{Clearly pas pour mo mais bon c'est comme ça. Après c'est quoi cumulative event ? Après ce qui est parfois perturbant c'est que temps en temsp c'est sur des cylindrical sets et des fois non. C'ets pas toujours simple de suivre la succession de remarques}.
...
\Cref{fig:copula_convex} illustrates the difference between $\Bel_\times$ and $\underline{P}$ in the case $n=2$\commanue{Peut-être commenter un plus donc on retrouve bien la relation démontrer plus haut.}    \commanu{Bravo pour le tikz, impressionné, sinon légende de la figure un peu chargée, plutot dans le texte en partie avec commentaires? pas très grave néanmoins mais en supplément de manue}.
...
Different models\commanue{juste le terme models tout seul ça fait un peu vague} can be used for\commanue{representing / characterizing ?} multiple sources of uncertainty. When some marginals are modeled using possibility distributions and other by p-boxes, it is still possible to derive results similar as those of sections \ref{sec:necessity_functions} and \ref{subsec:pboxes} if we use natural orders on the marginal focal sets\commanue{On ne peut mixer que possiblity et p-boxes, ou on peut faire la cuisine que l'on veut si on a un ordre naturel. Je dirais que l'intro permet de comprendre ce dont tu vas parler donc c'est un point positif. Mais je trouve que tu pourrais détailler un peu plus, c'est un peu rapide.}. 
...
\subsection{Using Other Orders}\commanue{donc là si j'ai bien compris c'est pas l'ordre naturel mais même modèles pour représenter l'incertitude. A voir si il faut pas adapetr le titre ou au moins l'intro car on comprend les conditions dans lesquelles tu te places avec l'exemple, c'est un peu tard.}\commanu{titre trop vague en effet}
...
\end{eqnarray*}
and $C(0,0)=0$ by continuity (we simplified the expression of the copula given in \Cref{tab:familiy_of_copula}). Let us consider $\X_1=\{x^1_1,\, x^1_2,\, x^1_3\}$, $\X_2=\{x^2_1,\, x^2_2,\, x^2_3\}$ and two possibility distributions $\pi_1$, $\pi_2$ over $\X_1$ and $\X_2$ respectively:
\begin{eqnarray*}
...
The \Cref{tab:beliefs_orders} shows a rounded value of $\Bel^{\preceq_1,\preceq_2}_\times$ for all orders for events $E_1$, $E_2$.
...
We ran simulation for the lower and upper Fréchet-Hoeffding copulas, as well as with the copulas presented in \Cref{tab:familiy_of_copula}. We were only able to find examples where no orders were working for the Ali-Mikail-Haq copula with $\theta>0$, for the Gumbel copula and for Clayton copulas. 
\end{example}
...
\commanu{idem pas facile à s'accrocher tout le long quand on est béotien. Important aussi de savoir ta contribution et l'état de l'art, pas évident de savoir en lisant. Et également de conclure pour savoir ce qu'apporte du coup cette partie généralement ou pour ton travail ensuite si ca peut etre fait avec les mains en conclu. Et aussi de mettre potentiellement des choses en annexes pour moins perdre le lecteur potentiellement? a toi de juger... Donc voir les remarques sur le fond de Seb, je n'oserais pas dire que je comprends tout dans sur ce chapitre ;)}
\pagebreak
\blankpage