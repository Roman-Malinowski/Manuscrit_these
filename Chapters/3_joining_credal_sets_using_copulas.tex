...
\begin{proof}
    ...
    For all \(i\in\opi 0,\, n\cli\) let $F_i$ be a CDF over $[0,\, N_i]$, with, $F_i(j)=\sum_{k=0}^j m_i(a_k^i)$. By Sklar's theorem, $F=C(F_1 \enum  F_n)$ is a multivariate CDF over $[0,\, N_1]\tdt[0,\, N_n]$. Thus it holds that $F([0,\, N_1]\tdt[0,\, N_n])=1$ and\commanue{Alors là j'ai du mal avec les notation. Après c'est une preuve donc c'est pas grave}:
    \begin{eqnarray*}
...
Constraints on this set only occur on cylindrical sets of product space $\X_1\tdt \X_n$ (from \cref{eq:cylindrical_sets})\commanue{je comprends pas ce que tu veux dire par constraints}, contrary to $\M_{mass}$ or $\M_{robust}$, we thus take the convex hull for extending its definition to probabilities on $\X$\commanue{en fait je ne comprends pas la phrase}. We note $\M_{agg}$ this credal set as it uses the copula as an aggregation operator only, without conserving the meaning associated to copulas by Sklar's theorem. In this regard, $\M_{agg}$ has less meaning than $\M_{robust}$ or $\M_{mass}$, but presents the advantage of being easier to compute on cylindrical sets. \Cref{fig:meaning_computation} sums up the performances of the different methods in terms of computation cost and meaningfulness.
...
\Cref{fig:copula_convex} illustrates the difference between $\Bel_\times$ and $\underline{P}$ in the case $n=2$\commanue{Peut-être commenter un plus donc on retrouve bien la relation démontrer plus haut.}.
